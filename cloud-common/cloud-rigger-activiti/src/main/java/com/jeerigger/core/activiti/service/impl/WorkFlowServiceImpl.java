...
import com.jeerigger.core.activiti.vo.CandidateVo;
import com.jeerigger.core.activiti.vo.OutgoingVo;
...
public class WorkFlowServiceImpl implements IWorkflowService {
    ...
    @Override
    public String deployByInputStream(String fileName, InputStream inputStream) {
        try {
    ...
    public boolean delete(String modelId) {
        try {
            ...
            log.error("流程模型删除失败", e);
            return false;
        }
    }

    @Override
    public String start(String processDefinitionKey, String initiator, String businessKey, Map<String, Object> variables) {
    ...
    @Override
    public int handleTask(String businessKey, String assignee, String assigneeName, String comments, String outgoing, Map<String, Object> variables) {
        try {
    ...
    @Override
    public String getWorkflowDesc(String workflowName) {
    ...
    @Override
    public boolean isComplete(String businessKey) {
        return null == runtimeService.createProcessInstanceQuery().processInstanceBusinessKey(businessKey).singleResult();
    }

    @Override
    public List<TaskVo> searchHistoryTaskByBusinessKey(String businessKey) {
        //获取流程发起人
        ...
        for (HistoricTaskInstance task : list) {
            ...
            if (!CollectionUtils.isEmpty(variableInstances)) {
                ...
                for (HistoricVariableInstance historicVariableInstance : variableInstances) {
                    if (ActivitiEnum.COMMENTS.getVariableType().equals(historicVariableInstance.getVariableName()) && null != historicVariableInstance.getValue()) {
                        taskVo.setComments(historicVariableInstance.getValue().toString());
                    }
                    if (ActivitiEnum.PASS.getVariableType().equals(historicVariableInstance.getVariableName()) && null != historicVariableInstance.getValue()) {
                        taskVo.setOutgoingName(historicVariableInstance.getValue().toString());
                    }
                    if (ActivitiEnum.ASSIGNEENAME.getVariableType().equals(historicVariableInstance.getVariableName()) && null != historicVariableInstance.getValue()) {
                        taskVo.setAssigneeName(historicVariableInstance.getValue().toString());
                    }
                }
            }
            taskVos.add(taskVo);
    ...
    @Override
    public int getToDoTaskNum(String assignee) {
        return taskService.createTaskQuery().taskAssignee(assignee).list().size();
    }

    @Override
    public List<OutgoingVo> getStartNodeByBusinessKey(String modelKey, String businessKey, String startUserId) {
        //流程走向集合
        ...
        if (null != flowElement && flowElement instanceof UserTask) {//下个节点是用户
            ...
            if (!CollectionUtils.isEmpty(candidateGroups)) { //用户组
                ...
                if (null != doc && doc.startsWith("{") && doc.endsWith("}")) {
                    ...
                    List<Map<String, Object>> maps = workFlowMapper.executeSql(doc);
    ...
    @Override
    public Page<Model> getModelByPage(PageHelper<Model> pageHelper, String modelName) {
    ...
    @Override
    public List<OutgoingVo> getOutgoingsByBusinessKey(String businessKey) {
        //流程走向集合
        ArrayList<OutgoingVo> outgoingVos = new ArrayList<>();
        Task task = taskService.createTaskQuery().processInstanceBusinessKey(businessKey).singleResult();
        if (null == task) {
    ...
    @Override
    public InputStream getResourceDiagramInputStream(String businessKey) {
...
//            Task task = taskService.createTaskQuery().processInstanceBusinessKey(businessKey).singleResult();
    ...
    @Override
    public boolean claim(String businessKey, String userId) {
        try {
            Task task = taskService.createTaskQuery().processInstanceBusinessKey(businessKey).singleResult();
    ...
    @Override
    public FileInputStream downloadModelByName(String modelName) {
    ...
    private List<String> getExecutedFlows(BpmnModel bpmnModel, List<HistoricActivityInstance> historicActivityInstances) {
        ...
        for (HistoricActivityInstance currentActivityInstance : finishedActivityInstanceList) {
            ...
            if (BpmActivityTypeEnum.PARALLEL_GATEWAY.getType().equals(currentActivityInstance.getActivityType())
                    || BpmActivityTypeEnum.EXCLUSIVE_GATEWAY.getType().equals(currentActivityInstance.getActivityType())) {
