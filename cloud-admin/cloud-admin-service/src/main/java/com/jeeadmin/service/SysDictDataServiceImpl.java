...
public class SysDictDataServiceImpl extends BaseServiceImpl<SysDictDataMapper, SysDictData> implements ISysDictDataService {
    ...
    public boolean updateStatus(Long dictId, String dictStatus) {
        SysDictData oldDictData = this.getById(dictId);
        if (oldDictData == null) {
    ...
    @Override
    public SysDictData saveDictData(SysDictData sysDictData) {
    ...
    public boolean updateDictData(SysDictData sysDictData) {
        ...
        SysDictData oldDictData = this.getById(sysDictData.getId());
        if (oldDictData == null) {
            throw new ValidateException("更新的字典数据不存在！");
        }
    ...
    private void validateDictValue(SysDictData sysDictData) {
        ...
        if (Objects.nonNull(sysDictData.getId())) {
            queryWrapper.lambda().ne(SysDictData::getId, sysDictData.getId());
        }
        queryWrapper.lambda().eq(SysDictData::getDictType, sysDictData.getDictType());
    ...
    public boolean deleteDictData(Long dictId) {
        SysDictData sysDictData = this.getById(dictId);
        if (sysDictData == null) {
            return true;
        } else {
