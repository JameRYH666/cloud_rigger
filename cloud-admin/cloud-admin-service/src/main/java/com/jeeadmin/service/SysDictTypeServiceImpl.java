...
public class SysDictTypeServiceImpl extends BaseServiceImpl<SysDictTypeMapper, SysDictType> implements ISysDictTypeService {
    ...
    public boolean updateStatus(SysDictType sysDictType) {
        SysDictType oldDictType = this.getById(sysDictType.getId());
        //验证字典类型是否存在
        if (oldDictType == null) {
        ...
        updateWrapper.lambda().eq(SysDictType::getId, sysDictType.getId());
    ...
    private void validateDictType(SysDictType sysDictType) {
        ...
        if (Objects.nonNull(sysDictType.getId())) {
            queryWrapper.lambda().ne(SysDictType::getId, sysDictType.getId());
        }
